...
func AddGeoDataPTHopBatch(hops []*schema.ParisTracerouteHop, timestamp time.Time) {
	...
	ips := make([]string, 2*len(hops))
	geo := make([]*api.GeolocationIP, 2*len(hops))
	for i := range hops {
		hops[i].Src_ip, _ = web100.NormalizeIPv6(hops[i].Src_ip)
		ips[i+i] = hops[i].Src_ip
		geo[i+i] = &hops[i].Src_geolocation
		hops[i].Dest_ip, _ = web100.NormalizeIPv6(hops[i].Dest_ip)
		ips[i+i+1] = hops[i].Dest_ip
		geo[i+i+1] = &hops[i].Dest_geolocation
	}

	annotation.AddGeoAnnotations(ips, timestamp, geo)
}
